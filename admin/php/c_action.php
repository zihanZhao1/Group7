...
if (isset($_POST["operate"]) && $_POST["operate"] == "addCourse") {
    ...
    foreach($arr as $key => $value){
        ...
        if($fname == 'Athletics Track'){
            $query1="insert into sei_booking(U_ID,F_ID,C_ID,start,end,count,Avb) values('$UID','$fid','$cid','$start_datetime','$end_datetime ',20,'no')";
			echo $query1;
            ...
            $num = implode("",$fnum);
            $query2="insert into sei_booking(U_ID,F_ID,C_ID,start,end,count,Avb) values('$UID','$fid','$cid','$start_datetime','$end_datetime','$num','yes')";
...
if (isset($_POST["operate"]) && $_POST["operate"] == "editCourse") {
    ...
    $statement->execute();
            ...
            foreach($arr as $key => $value) {
                ...
                $fname = $pdo->query("select name from sei_facility where F_ID = '$fid' ")->fetch(PDO::FETCH_ASSOC);
                if($fname == 'Athletics Track'){
                    $query2 = "insert into sei_booking (U_ID,F_ID,C_ID,start,end,count,Avb) values('$UID','$fid', ' $C_ID','$start_datetime','$end_datetime','20','no')";
                    $statement2 = $pdo->prepare($query2);
                    $statement2->execute();
                }else{
                    $fnum = $pdo->query("select capability from sei_facility where F_ID = '$fid' ")->fetch(PDO::FETCH_ASSOC);
                    $num = implode("",$fnum);
                    $query3 = "insert into sei_booking (U_ID,F_ID,C_ID,start,end,count,Avb) values('$UID','$fid', ' $C_ID','$start_datetime','$end_datetime','$num','yes')";
